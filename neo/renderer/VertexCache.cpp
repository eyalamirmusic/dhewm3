...
A real pointer. It used to be an offset into an ARB_vertex_buffer_object when
the cache had one, and the caller could not tell the two apart - which is why
the return type is void* and every caller casts it.
==============
*/
