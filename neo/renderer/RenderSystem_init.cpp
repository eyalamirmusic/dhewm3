...
idCVar r_useResidentGeometry( "r_useResidentGeometry", "0", CVAR_RENDERER | CVAR_BOOL, "keep static vertex cache blocks on the GPU instead of streaming them every frame (off: it costs more than it saves on this port's D3D12 driver - see \"Resident geometry\" in RenderBackend_Eacp.cpp)" );
...
idCVar r_windowResizable("r_windowResizable", "1", CVAR_RENDERER | CVAR_ARCHIVE | CVAR_BOOL, "Allow resizing (and maximizing) the window. Not implemented on the eacp host yet" );
...
void R_InitOpenGL( void ) {
	...
	// A Windows-only "please install OpenGL drivers from your graphics hardware
	// vendor" message box stood here, for Vista's Microsoft-supplied
	// OpenGL-over-D3D wrapper. It was reached through win32.osversion, which
	// went with the rest of Win32Vars_t, and it asks about a renderer this
	// build does not have.
}
